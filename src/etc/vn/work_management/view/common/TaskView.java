...
import etc.vn.work_management.model.work.Activity;
import etc.vn.work_management.model.work.Status;
...
public class TaskView {
    ...
    private void view() {
        ...
        if (WorkManagementApplication.isAdmin) {
            commonView.displayTaskList(workController.findAll());
        } else {
    ...
    private void createTask() {
        ...
        commonView.showMessage(CommonView.MESSAGE_TO_BACK);
        String title = commonView.inputString("- Nhập tiêu đề : ", scanner);
        while (true) {
        ...
        String description = commonView.inputString("- Tiếp theo nhập mô tả : ", scanner);
        Long idUser;

        if (WorkManagementApplication.isAdmin) {
            List<User> userList = userController.findAll();
            commonView.showMessage("- Tiếp theo là người làm task ");
            commonView.showMessage(CommonView.MESSAGE_TO_CHOOSE_ID);
            commonView.displayUserList(userList);
            String id = commonView.inputString("Mời bạn nhập id người làm task : ", scanner);
            while (true) {
                if (!validate.checkInputNumber(id)) {
                    commonView.showMessage(CommonView.MESSAGE_TO_REJECT_SELECTION);
                    commonView.showMessage(CommonView.MESSAGE_TO_CHOOSE_ID);
                    id = commonView.inputString("Mời bạn nhập id người làm task: ", scanner);
                } else {
                    if (userController.findUserById(Long.parseLong(id)).equals(null)) {
                        commonView.showMessage("Người làm task mà bạn chọn không tồn tại ");
                    } else {
                        break;
                    }
                }
            ...
            idUser = WorkManagementApplication.user.getId();
        }
        ...
        Task task = new Task(1L, title, description, idUser,
    ...
    private Activity inputActivity() {
        ...
        for (int i = 1; i <= Activity.values().length; i++) {
            commonView.showMessage(i + "." + Activity.values()[i - 1]);
        }
    ...
    private boolean updateStepTwo(Long id) {
        ...
        if (WorkManagementApplication.isAdmin) {
            commonView.showMessage("4.Câp nhật người làm task");
            max = 4;
        }
        String selection = commonView.inputString(CommonView.MESSAGE_TO_CHOOSE, scanner);
        while (true) {
            if (!validate.checkInputNumber(0, max, selection)) {
                commonView.showMessage(CommonView.MESSAGE_TO_REJECT_SELECTION);
                selection = commonView.inputString(CommonView.MESSAGE_TO_CHOOSE, scanner);
            } else {
                if (selection.equals("0")) {
    ...
    private void updateUserId(Long idTask) {
        Task task = workController.findById(idTask);
        List<User> userList = userController.findAll();
        commonView.showMessage(CommonView.MESSAGE_TO_CHOOSE_ID);
        commonView.displayUserList(userList);
        ...
        String id = commonView.inputString("Mời bạn nhập id người làm task : ", scanner);
        while (true) {
            if (validate.checkInputNumber(0, 0, id)) {
                commonView.showMessage("Cập nhật người làm task thất bại ");
                return ;
            }
            if (!validate.checkInputNumber(id)) {
                commonView.showMessage(CommonView.MESSAGE_TO_REJECT_SELECTION);
        ...
        task.setIdUser(Long.parseLong(id));
